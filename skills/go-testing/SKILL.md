...
- When refactoring or fixing Go code, consult `use-modern-go` first: run `sh "<skill-dir>/scripts/run-tool.sh" list --file-path <path>` (Windows: `'<skill-dir>\scripts\run-tool.ps1' list --file-path <path>`) or `--go-version 1.25`; prefer modern idioms surfaced by the CLI (e.g., `slices.Contains`, `min`/`max`, `range` over integers, `cmp.Or`, `errors.As` type patterns) when they match the change.
