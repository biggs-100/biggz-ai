...
### Added
- **subagent-live-activity** (2026-09-19) — you can now see what a subagent is *doing*, not just that it is alive (gentle-shell parity). The runtime derives a bounded activity label from the child's own RPC events (`activityLabel`: the announced tool plus its target, e.g. `read internal/install/steps/pi_extensions.go`, or `thinking` while it answers) and exposes it as `lastStep` on the task snapshot, so: the widget row above the editor reads `◐ sdd-explore · read pi_extensions.go · 42s` (falling back to the task summary before the first tool, and to the state when neither is known), and `subagent_status` / `subagent_list_tasks` answer "what is it doing?" instead of only "is it alive?". Foreground delegations publish the widget row too (they previously showed nothing while the TUI waited). Tests: 61/61 `node:test` (new: bounded activity label per event type; live step reaching the snapshot, the status line and the widget row; task-summary fallback).
- **subagent-agents-panel** (2026-09-19) — the rest of the gentle-shell visibility parity, in the same surface: the row now ends with the run's **spend** (tokens and cost accumulated from the child's own `usage` events, `formatTokens`/`formatCost`, `· 12.3k tok · $0.0042`), a **finished run stays visible for 60s** so its `✅`/`❌`/`⊘` is seen before it fades (`WIDGET_FINISHED_TTL_MS`, at most 3), and `elapsedMs` is **frozen at settle** (a finished run used to report a growing time, e.g. `completed · 38.0s` for a 12.9s task). New `/biggz-agents` command opens a minimal overlay panel (`ctx.ui.custom(..., { overlay: true })`): one row per run with live activity and spend, `↑↓` to move, `s` to stop the selected run (`stopped from the agents panel`), `q`/Esc to close; it repaints once a second and its `render` is total (a panel bug degrades to one line instead of freezing the TUI). Tests: 67/67 `node:test` (new: spend formatting, finished-row TTL, frozen elapsed, panel render/navigation/stop, command registration + non-TUI notice).

### Fixed
- **subagent-cancel-and-epipe** (2026-09-19) — Esc can now end a delegation, and a killed child's dead pipe can no longer throw inside pi. Two defects found while chasing a fresh hang: (1) the `subagent` tool ignored pi's `AbortSignal` (its third `execute` parameter, documented as the way Esc cancels extension work), so a running foreground delegation could not be cancelled at all — the TUI stayed on `Working…` and killing pi was the only exit; the tool now cancels the task tree on abort and reports `· cancelled by user (Esc) — the child tree was killed`; (2) the child's stdin had no `error` listener, so writing the prompt/abort to a child that had already exited raised an uncaught `EPIPE` inside the parent process (pi's TUI) on the settle/kill path — exactly where a delegation is supposed to report back. A foreground delegation also publishes the live widget row now (`◐ <agent> · running · <Ns>`), so a slow child reads as progress instead of a frozen screen. Evidence the child itself is healthy: the same task settles standalone through the runtime (12s), in headless foreground (9.9s) and as an in-TUI background run (12.7s), so the failure lives in the parent-side relay/settle path. Tests: 59/59 `node:test` (new Esc-abort case; the EPIPE leak failed the suite until the stdin listener landed); Go green.
- **child-agents-ask-policy** (2026-09-19) — children no longer ask the human. Live probes proved the second, worse half of the dialog defect: the relayed dialog is mounted **in place of the composer** with option 0 pre-highlighted and the keyboard focused, so any Enter the human pressed — including the Enter meant to send a chat message, which was then never sent — was swallowed as "select option 0" and the child acted on a decision nobody made (three probes settled `14.0s`, `123.4s`, `111.3s` reporting `Azul (Recommended)` / `"PROBE 3 visible?"="AZUL"` while the human was told to press Esc; mechanism verified in pi's own `extension-selector.js` + `interactive-mode.js`: only a confirm key resolves a value, countdown expiry and the runtime backstop both resolve `cancelled`). Policy now matches gentle-shell: (1) `internal/install/steps/pi_extensions.go` emits child agent tool lists **without** `ask_user_question` — asking is the orchestrator's job and a child that needs a decision returns `status: blocked`; (2) a background child's question is answered `{cancelled:true}` at once and never reaches the parent UI (`dialogPolicy: "dismiss"`, `dialogsDismissed` on the snapshot, logged as "dismissed without asking"); (3) a foreground question still relays, bounded as before, and now carries the asking agent id in the title (`sdd-propose: <question>`) so the human knows who is waiting. Tests: 3 new `node:test` cases (background dismissal never touches the UI, foreground relay carries the agent label, asker prefix in the parent dialog) — 58/58 green; Go green (`go test ./... -p 2`, `go vet`).
- **fix-child-dialog-hang** (2026-09-19) — a delegated child's question can no longer hang a delegation. Every `sdd-*` agent ships `ask_user_question`; in an RPC child that call emits `extension_ui_request{select}` and blocks until answered, and the runtime relay cleared the idle watchdog by design while it waited — so an unanswered question held the run until the 30-minute total bound and returned no tool result (reproduced with a real pi parent + `sdd-propose` child: child alive at t+24s, tool never returned; the user's own session `01a0b739-*.jsonl` ends on the same shape). The relay is now bounded: `DEFAULT_DIALOG_TIMEOUT_MS` (120s, `BIGGZ_SUBAGENT_DIALOG_MS` override, `0` = answer immediately) is passed to the parent dialog as its countdown budget with a runtime backstop timer, the child receives exactly one `{cancelled:true}` at the bound and continues, a late human answer never answers twice, the idle bound re-arms, and the unanswered question is announced on the parent UI, marked `awaiting answer: <title>` in the status line and reported in the `subagent` tool result (`dialogRelayed`/`dialogMissed`/`pendingDialog`/`dialogMs` on the snapshot). Verified live: an unanswered question at the shipped 120s default now settles `completed · 127.2s` with `PONG` plus the notice, where it previously hung. Tests: 5 new `node:test` cases (bound fires + run continues, idle bound stays honest, budget plumbed to the parent UI, `resolveDialogTimeout` clamping, tool-result notice) — 55/55 green. SDD artifacts in `openspec/changes/fix-child-dialog-hang/`.
- **pi-wrapper-removal** (2026-09-08) — Phase 1: `biggz install --agent pi` no longer deploys `biggz-memory-chrome.js`/`biggz-synthesis-gate.js` (67KB dead adapter-aware fallback; native-only `/mcp` via `pi-mcp-adapter@^2` is the only memory path). Install self-heals stale copies from existing `~/.pi/agent/extensions/`; JS sources kept on disk pending Phase-2 stability gate + one-release soak. `biggz-synthesis-gate.test.mjs` retired; fallback requirement removed from `pi-integration` spec. Go canonical gate (`internal/sdd/synthesis_gate.go`) unchanged. Tests: `TestPiExtensionsStep_DropsUnportableExtensions` pattern + factory mirror at 10 JS.
...
- **install-pi-websearch-yield** (2026-09-04) — `biggz-web-search.js` yields the `web_search` tool name when `pi-web-access` (or `pi-web-search`) is installed and registers `biggz_web_search` (DuckDuckGo no-key) as fallback instead, fixing the duplicate-tool crash on pi startup. Detection extracted as testable `providerSearchInstalled()`. Tests: 3 new `node:test` cases (12/12 green).
...
- **verify-complexity-gate** (2026-09-04) — `verify` phase now blocks on NEW cyclomatic>15/cognitive>20 offenders in code the change touched (critical packages only). Reuses the R2 lens hunk engine via exported `OffendersForFileDiffs`/`SplitFileDiffs`; grandfathering is structural (only diff hunks measured); non-git workspaces skip; `_test.go` warns and non-critical passes silent. Prevention: 15/20 budget line in `sdd-apply.md`. Evidence in the `complexity_gate` gatekeeper check (existing `sdd-gatekeeper` CLI). No git hooks. Tests: 19 new (lens/gate/gatekeeper incl. temp git repos); live dogfood pass+block proven. SDD artifacts in `openspec/changes/archive/2026-09-04-verify-complexity-gate/`.
