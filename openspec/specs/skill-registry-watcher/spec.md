# skill-registry-watcher Specification

## Purpose

Fsnotify watcher for skill registry providing sub-second mid-session regeneration with debounce, fingerprint gating, recursive existing-dirs watch, env/flag gating, poll fallback, and doctor integration.
...
The system MUST resolve targets via `uniqueExistingDirs` over 7 `ProviderPriority` (`user:opencode/biggz/claude/kilo` + `project:skills/opencode/github`), walk existing dirs recursively with `watcher.Add` for subdirs, `Add` new subdirs on `Create`, skip missing dirs without error.
