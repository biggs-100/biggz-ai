# odd-lane Specification

## Purpose

ODD (organic, non-SDD) lane: routing protocol with no SDD lifecycle, one recoverable task document under `odd/tasks/` at the repository root, and read-only document visibility in `biggz sdd-status`.
