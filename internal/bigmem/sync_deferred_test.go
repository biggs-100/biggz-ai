...
func TestDeferredFKMissing(t *testing.T) {
	...
	if err := s.ApplyPulledMutation(m); !errors.Is(err, ErrFKMissing) {
		t.Fatalf("expected ErrFKMissing got %v", err)
	}
	var cnt, att int
	var sid string
	...
	if cnt != 1 || att != 1 {
		t.Fatalf("cnt=%d att=%d sid=%q", cnt, att, sid)
	}
	if sid != pulledSessionDeadLetterSyncID("proj-deferred", "obs-deferred-1", pl) {
		t.Fatalf("sid %q want %q", sid, pulledSessionDeadLetterSyncID("proj-deferred", "obs-deferred-1", pl))
	}
}
