...
**Next Recommended:** none`;
  ...
  it('PR3 RED: translated markers block, Spanish content with English markers allows (parity with Go HasSynthesis whitelist)', async () => {
    const mock = createMockPi(); gateFn(mock);
    const h = mock._biggzSynthesisGate;
    const spanishContent = `## Sub-agent Result: sintesis
**What was done:**
| Topic | Decision |
|-------|----------|
| tema | decisión |
...
`;
    const translated = `## Resultado Sub-agente: sintesis
**Qué se hizo:**
| Tema | Decisión |
|-------|----------|
| a | b |
...
`;
    // hasSynthesis English whitelist: spanish content with English markers must PASS
    assert.equal(h.hasSynthesis(spanishContent), true, 'Spanish content with English markers must pass hasSynthesis');
    // translated markers must FAIL
    assert.equal(h.hasSynthesis(translated), false, 'translated markers must fail hasSynthesis (English whitelist via sanitizePlain)');
    // checkpoint with Spanish content synthesis should allow (30s window)
    delete process.env.BIGGZ_ADVISE;
    const checkpoint = { questions: [{ question: 'Next?', options: [{ label: 'Continuar' }, { label: 'Ajustar' }] }] };
    h._test.clearCurrent(); h._test.clearLast();
    h._test.setCurrent(spanishContent);
    let called = false;
    mock.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => { called = true; return { content: [{ type: 'text', text: 'ok' }] }; } });
    const w = mock._tools.get('ask_user_question');
    const resAllow = await w.execute('id-spanish', checkpoint, null, null, makeCtx('', []));
    assert.equal(resAllow.isError, undefined, 'Spanish content synthesis with English markers should allow checkpoint');
    assert.equal(called, true);
    // translated marker must block (missing English markers)
    h._test.clearCurrent(); h._test.clearLast();
    h._test.setCurrent(translated);
    called = false;
    const mock2 = createMockPi(); gateFn(mock2);
    mock2.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => { called = true; return { content: [{ type: 'text', text: 'ok' }] }; } });
    const w2 = mock2._tools.get('ask_user_question');
    // use h2's gate state
    const h2 = mock2._biggzSynthesisGate;
    h2._test.setCurrent(translated);
    const resBlock = await w2.execute('id-translated', checkpoint, null, null, makeCtx('', []));
    assert.equal(resBlock.isError, true, 'translated marker synthesis must block (missing English markers)');
    assert.equal(called, false);
    h._test.clearCurrent(); h._test.clearLast();
    h2._test.clearCurrent();
  });

  it('PR3 RED: streaming race immediate after message_end allows, missing blocks, history-only recall does not bypass', async () => {
    delete process.env.BIGGZ_ADVISE;
    const mock = createMockPi(); gateFn(mock);
    const h = mock._biggzSynthesisGate;
    const richMarkdown = `## Sub-agent Result: stream-test
**Artifacts/Paths:** internal/sdd/a.go, internal/sdd/b.go
**Risks / Open Questions:** none
**Next Recommended:** verify`;
    const checkpoint = { questions: [{ question: 'Next?', options: [{ label: 'proceed' }, { label: 'adjust' }] }] };
    const generalRecall = { questions: [{ question: 'Recall?', options: [{ label: 'opción A' }, { label: 'opción B' }] }] };
    // Pre-register tool before gate (load-order) is already handled by mock; now simulate streaming race via message_end
    let called = false;
    mock.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => { called = true; return { content: [{ type: 'text', text: 'ok' }] }; } });
    const w = mock._tools.get('ask_user_question');
    h._test.clearCurrent(); h._test.clearLast();
    // Simulate Pi message_end delivering markdown ms before tool_call
    const msgEnd = mock._onHandlers['message_end'];
    assert.ok(msgEnd, 'message_end handler must be registered');
    await msgEnd({ message: { role: 'assistant', content: [{ type: 'text', text: richMarkdown }] } });
    assert.ok(h._test.getCurrent().includes('Sub-agent Result'), 'message_end should populate currentTurn');
    const ctx = makeCtx('', []);
    const resImmediate = await w.execute('id-stream', checkpoint, null, null, ctx);
    assert.equal(resImmediate.isError, undefined, 'immediate after message_end (same turn <120s) must allow');
    assert.equal(called, true);
    // Missing currentTurn must block for checkpoint (even if history has recall)
    h._test.clearCurrent(); h._test.clearLast();
    h._test.setLast('## Session Recall\nhistory recall');
    called = false;
    const ctxMissing = makeCtx('## Session Recall\nhistory', []);
    const resBlock = await w.execute('id-stream-missing', checkpoint, null, null, ctxMissing);
    assert.equal(resBlock.isError, true, 'missing currentTurn with only history recall must still block for checkpoint');
    assert.equal(called, false);
    // History-only recall must not bypass checkpoint (narrow same-turn only)
    h._test.clearCurrent(); h._test.clearLast();
    // Set history but not currentTurn
    const handler = mock._getToolCallHandler();
    h._test.setLast('## Session Recall\nhistory');
    const ctxHist = makeCtx('## Session Recall\nhistory', []);
    const retHist = await handler({ toolName: 'ask_user_question', params: checkpoint }, ctxHist);
    assert.ok(retHist && retHist.block === true, 'history-only Session Recall must not bypass checkpoint (narrow same-turn only)');
    // Same-turn recall (currentTurn) must still bypass for non-checkpoint? For checkpoint before first synthesis, recall narrow allows? But hard gate fix says checkpoint still blocks unless recall same-turn? Actually JS gate allows checkpoint with same-turn recall? Check: tool_call checks checkSessionRecallInCurrentTurn -> allow. So we test same-turn recall allows checkpoint before first synthesis
    h._test.clearCurrent(); h._test.clearLast();
    h._test.setCurrent('## Session Recall\nsame-turn recall');
    const ctxSame = makeCtx('', []);
    const retSame = await handler({ toolName: 'ask_user_question', params: checkpoint }, ctxSame);
    // same-turn recall should allow (narrow exception before first synthesis)
    assert.equal(retSame, undefined, 'same-turn Session Recall should allow checkpoint (narrow pre-first-synthesis exception)');
    h._test.clearCurrent(); h._test.clearLast();
  });

  it('PR3 RED: child admission still blocks checkpoint in child, thin advise parity', async () => {
    const checkpoint = { questions: [{ question: 'Next?', options: [{ label: 'proceed' }, { label: 'adjust' }] }] };
    const thinMarkdown = `## Sub-agent Result: thin
**Artifacts/Paths:** -
**Risks / Open Questions:** none
**Next Recommended:** none`;
    const richMarkdown = `## Sub-agent Result: rich
**Artifacts/Paths:** internal/assets/pi/biggz-synthesis-gate.js (dual-mode watchdog with heuristic and BIGGZ_ADVISE gate), internal/assets/pi/biggz-web-search.js (anchor-preserving markdown fetch with baseUrl resolve), internal/tui/tui.go (CSI sync and bracketed paste handling with fallback)
**Risks / Open Questions:** none
**Next Recommended:** verify`;
    // Child bypass: general ask should allow even missing, but child checkpoint must still be blocked (single ownership)
    // gate must be loaded as orchestrator (no child at load time), then child flag set at call time for runtime bypass test
    delete process.env.BIGGZ_ADVISE;
    delete process.env.PI_SUBAGENT_CHILD;
    const mock = createMockPi(); gateFn(mock);
    const h = mock._biggzSynthesisGate;
    process.env.PI_SUBAGENT_CHILD = '1';
    assert.equal(h.isChildBypass(), true);
    let called = false;
    mock.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => { called = true; return { content: [{ type: 'text', text: 'ok' }] }; } });
    const w = mock._tools.get('ask_user_question');
    h._test.clearCurrent(); h._test.clearLast();
    const ctxMissing = makeCtx('no markers', []);
    const general = { questions: [{ question: 'general?', options: [{ label: 'A' }, { label: 'B' }] }] };
    const resGeneralChild = await w.execute('id-child-general', general, null, null, ctxMissing);
    assert.equal(resGeneralChild.isError, undefined, 'child general ask should allow even missing');
    assert.equal(called, true);
    called = false;
    h._test.clearCurrent();
    const resCpChild = await w.execute('id-child-cp', checkpoint, null, null, ctxMissing);
    assert.equal(resCpChild.isError, true, 'child checkpoint must still be blocked (single ownership)');
    assert.equal(called, false);
    // Thin parity: thin with BIGGZ_ADVISE=1 should warn but allow; without advise silent
    delete process.env.PI_SUBAGENT_CHILD;
    process.env.BIGGZ_ADVISE = '1';
    const mock2 = createMockPi(); gateFn(mock2);
    const h2 = mock2._biggzSynthesisGate;
    mock2.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => ({ content: [{ type: 'text', text: 'ok' }] }) });
    const w2 = mock2._tools.get('ask_user_question');
    h2._test.setCurrent(thinMarkdown);
    assert.equal(h2.isThinSynthesis(thinMarkdown), true);
    const ctxThin = makeCtx(thinMarkdown, []);
    const notifyBefore = mock2._notifyCalls.length;
    await w2.execute('id-thin-advise', { questions: [{ question: 'Q?', options: [{ label: 'A' }, { label: 'B' }] }] }, null, null, ctxThin);
    const hasConcern = mock2._notifyCalls.slice(notifyBefore).some(n => String(n.msg).includes('concern'));
    assert.ok(hasConcern, 'thin with BIGGZ_ADVISE=1 should emit concern');
    // Without advise silent
    delete process.env.BIGGZ_ADVISE;
    const mock3 = createMockPi(); gateFn(mock3);
    mock3.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => ({ content: [{ type: 'text', text: 'ok' }] }) });
    const h3 = mock3._biggzSynthesisGate;
    h3._test.setCurrent(thinMarkdown);
    const before3 = mock3._notifyCalls.length;
    await mock3._tools.get('ask_user_question').execute('id-thin-silent', { questions: [{ question: 'Q?', options: [{ label: 'A' }, { label: 'B' }] }] }, null, null, makeCtx(thinMarkdown, []));
    const hasConcernSilent = mock3._notifyCalls.slice(before3).some(n => String(n.msg).includes('concern'));
    assert.equal(hasConcernSilent, false, 'thin without advise must be silent');
    // Rich never concern even with advise
    process.env.BIGGZ_ADVISE = '1';
    const mock4 = createMockPi(); gateFn(mock4);
    mock4.registerTool({ name: 'ask_user_question', description: 't', parameters: { type: 'object', properties: {} }, execute: async () => ({ content: [{ type: 'text', text: 'ok' }] }) });
    const h4 = mock4._biggzSynthesisGate;
    h4._test.setCurrent(richMarkdown);
    assert.equal(h4.isThinSynthesis(richMarkdown), false);
    delete process.env.BIGGZ_ADVISE;
    delete process.env.PI_SUBAGENT_CHILD;
  });
});
