...
Before losslessly relaying any blocking choice envelope, classify its semantic admissibility. When the consumer workflow appears blocked by a biggz-ai provider or tool defect, never offer to switch to, inspect, modify, or directly repair the biggz-ai repository from that workflow. If an upstream envelope offers direct repair, do not silently mutate it: reject it as semantically inadmissible and issue this separate orchestrator-owned handoff envelope.
...
- On a consented report path, prepare or reuse privacy-scrubbed diagnostics. Immediately before the first GitHub operation, perform a final privacy scan. This scan precedes the duplicate search, report creation, and occurrence comment. Exclude raw argv, absolute paths, private project names, usernames, hostnames, credentials, diffs, source contents, and environment values.
  1. **Report the biggz-ai defect and continue**: Only after explicit consent and that final privacy scan, search open and closed issues in `biggs-100/biggz-ai`.
      ...
      - If an equivalent issue exists, add one new occurrence comment with the observed evidence only on that exact issue; do not add, remove, or change any labels on it. If no equivalent issue exists, file a new plain provider-defect issue in `biggs-100/biggz-ai`. No label is applied automatically: biggz-ai has no issue-filing label automation.
      - Confirmed creation is a HARD precondition: only a GitHub create operation that confirms a newly-created issue identity/URL counts as a filed report. Never infer creation from output text alone. If creation fails, is ambiguous, incomplete, times out, lacks permission, or has an unknown outcome, STOP with all consumer state preserved. Do not search, comment, update, label, or retry creation until the exact created issue identity is resolved.
      - If creation is confirmed, the report is complete: surface the confirmed created issue identity/URL and STOP with all consumer state preserved; do not create or comment again automatically.
      - On retry, perform a fresh final privacy scan first, then re-resolve that exact created issue identity. Never search and file against an arbitrary equivalent/pre-existing issue. If the exact created issue identity cannot be proven, STOP and require a human decision, with no duplicate issue/comment. Then STOP with all consumer state preserved.
...
- Both continue choices execute that exact captured decline invocation exactly once: use only the exact captured provider-owned `choices[id="declined"].invocation` from the `biggz-ai.review-consent/v1` envelope. Never synthesize the decline command, target, token, or consumer continuation from prose.
- If the captured exact decline invocation, exact target identity, or consumer continuation context is unavailable or ambiguous, fail closed with all consumer state preserved and do not run a substitute command.
- On a successful exact decline, validate the candidate-scoped decline confirmation (exact target identity match, nothing persisted); then re-enter through native negotiated STATUS, then resume the already-held consumer continuation.
