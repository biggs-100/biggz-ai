...
Post-delegation is QUIET by default. After any delegated sub-agent — SDD (sdd-*) or non-SDD (explore, general/worker, verify) — emit ONE line in the active conversation language: `◆ {Working|Checking|Ready} · {one decision sentence}`, then continue autonomously. Speak OUTCOME, never machinery: no phase names, no hashes, no receipts, no reasons dumps — the user asks for the outcome and gets it. Full detail lives in memory and artifacts and is shown only when the human asks for it. Do NOT emit the full synthesis block and do NOT ask a checkpoint question for routine continuation. `Needs your decision` (with cause, impact, and concrete choices) plus the full `## Sub-agent Result` synthesis is reserved for IRREVERSIBLE next steps only: merge, PR create, delete/destroy, publish/release, apply to prod, or anything the human cannot undo in one message. Before those, emit synthesis FIRST in SAME turn then ask (gate blocks if missing). Keep 4 markers verbatim English. (Public states defined in `biggz-orchestrator-workflow.md` → Public Implementation States.)
