...
Sweep budget: one exhaustive sweep, at most two for >400 lines.
...
The controller records `approved` only when no severe rows survive and final verification passes. Otherwise it records `escalated`. Actor prose is never terminal authority. Sweep budget: one exhaustive sweep, at most two for >400 lines. Two blind read-only judges, zero refuters, at most two fix rounds, findings surviving round two escalate.
