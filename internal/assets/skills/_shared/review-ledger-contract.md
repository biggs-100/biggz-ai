...
- `collect` — satisfy the named capture input with its exact capture operation: materialize the reviewer task with `biggz review capture-result --materialize` (the host plugin runs the `--preflight` validation and the materialize composition, then transports the bytes verbatim), run the reviewer on exactly those bytes, capture with the same binding via `--input -`, then query STATUS again. `subject_hash` is intentionally omitted from the envelope: the transport derives it from the preflight before the real capture.
...
The reviewer runs as one foreground `task` call whose prompt is provider-owned: the `review-result-artifacts` plugin materializes the complete reviewer task in Go and forwards those bytes verbatim — the materialized task IS the reviewer prompt, so every caller-authored byte after the binding line is discarded. The reviewer runs tool-less: the shipped review agents deny every tool, so the frozen bytes are the only evidence it inspects. The plugin is deployed by `biggz install` to the agent's plugin directory (`~/.config/opencode/plugins/` for OpenCode), which auto-loads it at startup; it is the transport between the orchestrator, the reviewer sub-agent, and the native capture CLI.

**Binding.** Begin the reviewer task prompt with the exact literal prefix `GENTLE_AI_REVIEW_BINDING ` — including the trailing space and never `=`; the same literal as gentle-ai, the de-facto standard across both projects — followed by one-line JSON assembled only from provider-returned state: `lineage`, `target` (subject commit), `lens`, `order`, `revision` from the lineage head (`expected-revision`), `repository_context` when provider-issued, and `subject_hash` when the artifact subject is already known; omit only provider-omitted fields. These are the prompt's first bytes and the only caller-authored input the transport keeps. Never add `candidate_diff` or candidate bytes — the reviewer receives only the provider-materialized task.

**Preflight.** The plugin's `tool.execute.before` hook rejects background tasks and malformed bindings, then runs `biggz review capture-result --preflight` with the binding flags (no `--cwd` — biggz resolves the repository from the working directory; a provider-issued `repository_context` may pin it). The returned artifact subject (base/candidate trees + ordered changed-path manifest) fixes the exact frozen view the capture leg asserts.

**Materialize.** The same hook then runs `biggz review capture-result --materialize` with the same binding flags and replaces the caller's task body with the printed bytes, unchanged: binding, context, name-status, numstat, then one `GENTLE_AI_REVIEW_PATCH <path>` section per changed path in manifest order. The host forwards the bytes byte-for-byte — no trim, no re-encode, no augmentation — and proves the materialized binding still describes the preflighted subject before any reviewer launches. Materialization is read-only and refuses whole (typed) when the task is vacuous or exceeds the byte cap; it never truncates.

**Capture.** The reviewer echoes `subject_hash` and returns completed inspection over every manifest path, findings, and evidence. The plugin's `tool.execute.after` hook extracts strict JSON from the task result — rejecting empty and nested envelopes — runs `biggz review capture-result --input -` with the binding flags, and replaces the task output with the captured artifact. The capture leg keeps the provider-issued `repository_context` and the preflight-derived `subject_hash`, so verbatim transport never weakens it.
