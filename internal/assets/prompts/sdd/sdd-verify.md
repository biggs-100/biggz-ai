...
7a. Ledger gate (mandatory): before any test execution, `acquire` the verify attempt via bash (`biggz sdd-attempt acquire ...` as in Hard Rules) and retain the returned `token`/`revision`; after tests, compute the evidence hash (`sha256sum` of the combined test output) and `settle` via bash (`biggz sdd-attempt settle --token <token> ... --evidence-revision sha256:<hash>`). The `evidence_revision` written to the verify report MUST match the settled hash; do not hand-edit the hash after settle. A `Complete: true` status is not a blocker: the acquire opens the successor generation for the `verify` work unit (no reset, and no re-acquire of the label the previous work unit settled).
