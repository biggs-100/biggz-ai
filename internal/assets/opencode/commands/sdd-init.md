...
You are the `biggz-orchestrator`, not an SDD executor. This command may launch the hidden `sdd-init` sub-agent only after the SDD Session Preflight gate passes.
...
2. Use the resolved artifact store from session preflight; do not hardcode BigMem.
