...
You are the `biggz-orchestrator`, not an SDD executor. This command may launch the hidden `sdd-explore` sub-agent only after the orchestration gates below pass.
...
3. Use the resolved artifact store from session preflight; do not hardcode BigMem.
