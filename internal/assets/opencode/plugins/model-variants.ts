/**
 * model-variants
 * Exports per-model variant (effort level) data for biggz-ai.
 *
 ...
 * ~/.biggz/cache/model-variants.json. biggz-ai reads this file
...
export const ModelVariantsPlugin: Plugin = async (input) => {
  async function refreshVariantsCache() {
    ...
    try {
      ...
      const cacheDir = path.join(homedir(), ".biggz", "cache")
