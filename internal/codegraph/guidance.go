// Package codegraph provides the shared CodeGraph guidance text and
// project-root validation for the `biggz codegraph` verb.
package codegraph

import "strings"

// GuidanceMarkdown is the instruction block injected into agent system prompts
// when CodeGraph is enabled. Adapted from gentle-ai's communitytool guidance
// (biggz has no bench/ corpus — black-box journeys replaced by e2e).
func GuidanceMarkdown() string {
	return strings.Join([]string{
		...
		"- Do not use `biggz codegraph` as a general proxy. Its `init` command exists only to validate the project root before initialization; intelligence queries belong to the upstream CLI.",
		...
		"4. If `.codegraph/` is missing and CodeGraph is enabled/available, immediately run `biggz codegraph init --cwd <project-root>` once.",
