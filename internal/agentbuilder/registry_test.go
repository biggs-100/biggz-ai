...
func TestSaveAndLoadRegistry_RoundTrip(t *testing.T) {
	...
	if loaded.Agents[0].GenerationEngine != agents.AgentClaudeCode {
		t.Errorf("GenerationEngine = %q, want %q", loaded.Agents[0].GenerationEngine, agents.AgentClaudeCode)
	}
}
...
func TestHasConflictWithBuiltin_TrueForBuiltin(t *testing.T) {
	builtins := []string{"code-review", "mcp-tools", "auto-fix", "Code Review", "MCP Tools"}
