...
func TestComposePrompt_InstalledAgentsIncluded(t *testing.T) {
	agentIDs := []model.AgentID{agents.AgentClaudeCode, agents.AgentOpenCode}
	prompt := ComposePrompt("build an agent", nil, agentIDs)
	...
	if !strings.Contains(prompt, string(agents.AgentClaudeCode)) {
		t.Errorf("should include claude-code agent; got:\n%s", prompt)
	}
	if !strings.Contains(prompt, string(agents.AgentOpenCode)) {
...
func TestComposePrompt_ReferencesBigMemNotEngram(t *testing.T) {
	prompt := ComposePrompt("build an agent", nil, nil)

	if !strings.Contains(prompt, "BigMem Integration") {
		t.Errorf("should reference BigMem integration; got:\n%s", prompt)
	}
	if !strings.Contains(prompt, "biggz_mem_save") {
		t.Errorf("should reference biggz_mem_save tool; got:\n%s", prompt)
	}
	if strings.Contains(prompt, "Engram") {
		t.Errorf("should not reference Engram; got:\n%s", prompt)
	}
}
