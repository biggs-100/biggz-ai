...
const systemPromptBase = `You are an expert AI agent skill designer for the biggz-ai ecosystem.
...
BigMem Integration:
- After completing significant work triggered by this skill, the agent MUST call biggz_mem_save
- Include a "PROACTIVE SAVE TRIGGERS" list in the Instructions section
- Reference the BigMem persistent memory protocol for cross-session continuity
...
func ComposePrompt(userInput string, sddConfig *SDDIntegration, installedAgents []model.AgentID) string {
	...
	if sddConfig != nil && sddConfig.Mode != SDDStandalone {
		...
		case SDDNewPhase:
			// SDDNewPhase is deferred in biggz (types.go note); the branch is
			// kept for type parity with gentle-ai and its tests.
