...
func TestIntegration_FullAgentBuilderFlow(t *testing.T) {
	...
	installedAgents := []model.AgentID{agents.AgentClaudeCode}
	...
	engine := &MockEngine{
		AgentIDVal:  agents.AgentClaudeCode,
		Output:      cannedSKILL,
		IsAvailable: true,
	}
	...
	adapters := []AdapterInfo{
		{AgentID: agents.AgentClaudeCode, SkillsDir: dir1},
		{AgentID: agents.AgentOpenCode, SkillsDir: dir2},
	}
	...
	reg.Add(RegistryEntry{
		...
		GenerationEngine: agents.AgentClaudeCode,
		InstalledAgents:  []model.AgentID{agents.AgentClaudeCode, agents.AgentOpenCode},
	})
	...
	if found.GenerationEngine != agents.AgentClaudeCode {
		t.Errorf("registry entry GenerationEngine = %q, want %q", found.GenerationEngine, agents.AgentClaudeCode)
	}
}
