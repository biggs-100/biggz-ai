// Package windsurf implements the AgentAdapter interface for Windsurf
...
func init() {
	agents.Register(agents.AgentWindsurf, func() plugin.AgentAdapter { return NewAdapter() })
}
...
func (a *Adapter) Detect(ctx context.Context, homeDir string) (bool, string, string, bool, error) {
	...
	if err != nil {
		return false, "", "", false, fmt.Errorf("windsurf: not found: %w", err)
	}
...
func (a *Adapter) SettingsPath(homeDir string) string {
	return filepath.Join(homeDir, ".codeium", "windsurf", "settings.json")
}
