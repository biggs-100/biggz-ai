...
├── assets/          — Embedded skills + configs + OpenCode plugins (opencode plugins + biggz-orchestrator.md [internal/assets/biggz/] + pi extensions [internal/assets/pi/]; wrappers `biggz-memory-chrome.js` + `biggz-synthesis-gate.js` stay embedded but are excluded from the pi deploy list — native-only `/mcp`, sources kept pending Phase-2 gate)
...
Guarantees the human sees audited `## Sub-agent Result` before deciding. Gate `b0d2fc1` enforces the Post-Delegation Human Checkpoint. Session Boot Recall (`## Session Recall`) is a preceding hard gate that restores prior session context via BigMem before preflight. Synthesis rendering uses `internal/sdd/synthesis.go` `RenderSynthesis` / `RenderSynthesisLocalized` (with `DetectLanguage` heuristic) with 4 required markers + 6 optional (Preview/Diff/Decisions/Commands/Validation/Failure), omit-empty, >50KB `ReadLoop` paginated `ReadAt` with verify-retry, `ValidateQuestionEnvelope` 16/60/4/2-4, `pending` dual-write BigMem + `state.yaml` (`biggz-ai.pending-question/v1`), Go canonical `internal/sdd/synthesis_gate.go` `ShouldBlock`/`CheckSynthesisPrecondition`/`BuildBlockedEnvelope` + `ShouldBlockApplyAdmission` (write-admission ignores child/recall) is the enforced rule core, reached at ask time by the deployed path `ask-user-choice.ts` → `biggz-ask-guard.js` → `biggz sdd-ask-check` (argv array, no shell, 1000 ms bound, stdin JSON): a decided block refuses to present the question and surfaces the check message, an indeterminate check (timeout, missing/stale binary) degrades visibly with a warning. The Pi-side `biggz-synthesis-gate.js` dual path (`{isError:true}`/`{block:true}`, `PI_SUBAGENT_CHILD=1` narrow bypass, `BIGGZ_ADVISE=1` thin-advise) is no longer deployed (excluded from `piExtensionsDeployList` + stale self-heal; source kept on disk pending Phase-2 gate).

**Visual design (cognitive-doc-design):** Template follows *scannable hierarchy* — chunked `###` headers with consistent icons (📋 What was done, 📦 Deliverables, ⚠️ Risks, ➡️ Next Step, 🔍 Preview & Changes, 💡 Decisions & Commands, ✅ Validation, ❌ Failure), *progressive disclosure* (happy path What→Deliverables→Next first, evidence/validation after), *recognition over recall* (tables `| Topic | Decision |` + checklists `- [x]` over prose), and *signposting* (colored lifecycle `◆ phase · status · next`). Verbatim English markers (`## Sub-agent Result:`, `**Artifacts/Paths:**`, `**Risks / Open Questions:**`, `**Next Recommended:**`, `| Topic | Decision |` / `**What was done:**`) remain invariant for gate `b0d2fc1`; `###` headers and emoji are visual layer only and never translate markers (paths/code stay English). See `internal/assets/biggz/biggz-orchestrator.md` copy-paste block for canonical visual order.

**Language Boundary (harness vs artifact):** Harness prompts stay English; synthesis content is localized per human language (`languageHint` / `Human language: es|en — render synthesis content in that language, keep markers English, keep paths/code English`), stored via `pending_question.languageHint` dual-write (`BigMem sdd/{change}/pending-question` + `state.yaml`, `biggz-ai.pending-question/v1`) and injected into every `sdd-*` prompt with fallback `DetectLanguage(lastHumanMessage)` or `en` at render via `RenderSynthesisLocalized`. Markers (`## Sub-agent Result:`, `**Artifacts/Paths:**`, `**Risks / Open Questions:**`, `**Next Recommended:**`, `| Topic | Decision |`) and technical identifiers (paths, `sdd/...`, `ORDER BY`, `Search`, code, branches, topic_keys) stay English — gate `b0d2fc1` (`HasSynthesis`/`isCheckpointAsk`) validates verbatim English markers; whitelist via `sanitizePlain` never translates them. `isCheckpointAsk` scans option labels only, not synthesis content, so Spanish synthesis with English markers passes; translated marker (`**Artefactos/Rutas:**`) fails and blocks regardless of language.
...
**Layer 2 — Go canonical + deployed ask check (blocking) + retired Pi gate source (advisory only):** `internal/sdd/synthesis_gate.go` is canonical: `HasSynthesis` requires 4 markers (`## Sub-agent Result`, `**Artifacts/Paths:**`, `**Risks / Open Questions:**`, `**Next Recommended:**` plus `**What was done:**` OR `| Topic | Decision |`), `IsCheckpointAsk` scans ONLY option `label`/`value`/`id`/`name`/`title` bilingual tokens (`proceed|continuar|proseguir|proceder|procede`, `adjust|ajustar`, `stop|detener|parar`, `continue|continuar`, `correct|corregir|cerrar`), body text never signals; `HasOptions` alone never blocks; `ShouldBlock(question, md, now)` = `!IsChildBypass && !HasSessionRecall(currentTurn) && IsCheckpointAsk && (HasSynthesis(currentTurnMarkdown)==false || now-currentTurnTime>120s)` where `currentTurnMarkdown` is STRICT same-turn buffer via `SetCurrentTurnMarkdown` reset on `turn_start`/`agent_start`/`tool_execution_end`; `CheckSynthesisPrecondition` + `BuildBlockedEnvelope`/`blockedEnvelope` emit `context` + `FormatFallback` verbatim on block; `ShouldBlockApplyAdmission` ignores child/recall to prevent auto back-to-back phases self-validating write entry. `internal/assets/pi/biggz-synthesis-gate.js` (source-only, NOT deployed to `~/.pi/agent/extensions/` — see `pi-wrapper-removal` note at the end of this bullet) mirrors Go strictly: wraps `pi.registerTool` + sweeps pre-registered `pi.tools`/`_tools`/`getAllTools→getToolDefinition` (load-order safe), hooks `pi.on("tool_call")` as secondary `{block:true}`, strict `currentTurnMarkdown` only (history never satisfies), `message_end`/`message_update`/`assistant_message` buffer + `turn_start`/`agent_start` reset fixes streaming race (markdown ms before tool_call, 120s window), thin `countPaths<2||len<50` warn only `BIGGZ_ADVISE=1`, narrow `## Session Recall` same-turn bypass only before first synthesis (history recall must not bypass after delegation), `PI_SUBAGENT_CHILD=1` bypass except child checkpoint still blocked and admission ignores bypass. (`pi-wrapper-removal`: the `internal/assets/pi/biggz-synthesis-gate.js` mirror above is source-only — it is NOT deployed to `~/.pi/agent/extensions/` and stale copies are self-healed; in-harness fail-closed enforcement is absent until a native replacement, Go canonical unchanged.)
...
- **Blocking (default, checkpoint-only; live path `ask-user-choice.ts` → `biggz-ask-guard.js` → `biggz sdd-ask-check`):** when the 4-marker check fails **and** `isCheckpointAsk(params)` is true, the gate **blocks** in BOTH paths: (a) wrapped `execute` returns `{content:[{type:"text", text:"Please synthesize before asking — missing ## Sub-agent Result block..."}], isError:true}` and does **not** call `original()`, (b) `tool_call` handler returns `{block:true, reason}` — either path prevents the question reaching the user. General asks (`isCheckpointAsk` false) bypass blocking entirely (allowed directly). Both blocked paths notify via `pi.notify`/`ctx.ui.notify`. Synthesis inside the tool's `question` param does not satisfy the check. Checkpoint blocks even with no prior synthesis ever (hard gate fix — only Session Recall narrow exception before first synthesis).
...
- **Session Recall exception (narrow):** before the first synthesis, a same-turn `## Session Recall` block satisfies the gate for the subsequent ask. Only `currentTurnMarkdown` containing `## Session Recall` counts; history does not. After the first `## Sub-agent Result`, strict same-turn synthesis is required and Session Recall no longer bypasses.
- **Bypass:** only `PI_SUBAGENT_CHILD=1` bypasses both modes except child checkpoint still blocked (`isError:true checkpoint asks may only be emitted by orchestrator`) and `ShouldBlockApplyAdmission` ignores child/recall for write admission. There is no orchestrator bypass (`BIGGZ_ORCHESTRATOR` is not honored).
- **Checkpoint vs general:** The synthesis gate checkpoint applies ONLY to Post-Delegation Human Checkpoint questions (`proceed`/`adjust`/`stop` or `continue`/`correct` after a delegated sub-agent). The JS gate detects this via `isCheckpointAsk(params)` (case-insensitive scan of `params.questions[].options[].label` and deep `label`/`value`/`id`/`name` fields). General clarification questions that are NOT a checkpoint (no such labels, null/undefined params, or unrelated options) do NOT require synthesis and MUST NOT be blocked — both `wrapSingleTool` and `tool_call` guards `return` early allowing the call directly (optionally advise if thin but never block), matching prompt authority.
...
- **Unit Go:** `internal/sdd/synthesis_gate_test.go` + `internal/sdd/transcript_lint_test.go` (`go test ./internal/sdd -run TestShouldBlock|TestTranscript`) covers REQ-DG-1/2 + REQ-SDD (`HasSynthesis` 4 markers+table, label-only vs body, `HasOptions` never blocks, 120s/turn-reset, translated marker fails, streaming race via `message_end` 120s, `HasSessionRecall` same-turn narrow vs history, `ShouldBlockApplyAdmission` child/recall ignored for admission), plus transcript lint replaying `branch-worktree-cleanup` 8-phase fixture (2 syntheses→7 blocks, complete→0).
- **Unit JS (retired with wrapper):** `internal/assets/pi/biggz-synthesis-gate.test.mjs` deleted (`pi-wrapper-removal` Phase 1); Pi-side gate coverage retires with the wrapper. Remaining coverage: Go `synthesis_gate_test.go` + `transcript_lint_test.go` (canonical gate), `orchestrator_test.go` (prompt markers), `biggz-pi-extensions-factory.test.mjs` (`node --test`, deployed-list factory shape).
- **Integration:** `internal/assets/biggz/orchestrator_test.go` (`go test ./internal/assets/biggz`) reads the embedded `biggz-orchestrator.md` via `assets.FS` and asserts the 4 synthesis markers, `INVALID and will be blocked`, 12× `REMINDER`, and the `## Session Recall` hard gate with `biggz_mem_context`/`biggz_mem_search`/`sdd-status` fallback, failing on drift. `docs/architecture.md` itself asserts blocking contract with 3-layer defense + `INVALID` and no retired passthrough.
- **CI:** `go vet ./...`, `go test ./...`, `node --check` on remaining pi extensions + `node --test internal/assets/pi/biggz-pi-extensions-factory.test.mjs` must be green; `synthesis-gate-status` command reports `✓`/`⚠`/`✗` in the TUI.
