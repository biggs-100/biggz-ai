...
> ENFORCEMENT RETIRED (2026-09-04, code removed 2026-09-05): blocking proved unfulfillable from the
> agent side (same-turn side-channel + body-text false positives). Blocking functions
> (`ShouldBlock`/`ShouldBlockApplyAdmission`/`BuildBlockedEnvelope`) and dead wrapper paths are deleted;
> only pure advise helpers (`HasSynthesis`, `isCheckpointAsk` labels-only, `FormatFallback`)
> and their unit tests stay as living documentation. Context-before-question is governed by the
> explicit ask contract in the orchestrator delegation doc, not by code.
>
**Layer 2 — Pi gate (passthrough + advise helpers):** `internal/assets/pi/biggz-synthesis-gate.js` wraps `pi.registerTool` for future `ask_user_question`/`question` AND sweeps pre-registered tools (`pi.tools` / `pi._tools` / `pi.getAllTools`→`getToolDefinition` if available) — load-order safe — and hooks `pi.on("tool_call")` as a secondary guard. Both paths are passthrough since 2026-09-04 (dead blocking paths removed 2026-09-05); helpers stay exposed for tests.
