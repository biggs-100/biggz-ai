...
- **Unit JS:** `internal/assets/pi/biggz-synthesis-gate.test.mjs` (`node --test`) covers checkpoint-gated blocking (`isError:true` + `{block:true}`), thin advise (`BIGGZ_ADVISE=1` warn), child bypass vs child admission block, same-turn race, history-only block, `turn_start` reset, load-order sweep, `message_end` buffer, Session Recall narrow vs history, checkpoint vs general (bilingual `proceder`/`procede`), parity vs Go (`blockedEnvelope` `context`+`fallback` verbatim, `HasSynthesis` markers English whitelist, `IsCheckpointAsk` label-only), translated marker and recall-outside-current threat cases.
