...
| Full management TUI (model picker, Configure Models, agent set editing) | ✅ Harness-focused TUI (30 files vs gentle's installer-focused 104): agent-builder + 4-mode model picker with variants cache + `memory` graph (`g` toggle) + `sdd-continue` picker + `sdd-status --watch` + `BIGGZ_NO_ANIMATION` reduced-motion (9becc12). Both have TUIs; the focus differs: gentle's TUI installs/configures agents (agent_builder flow, per-agent pickers), biggz's operates SDD/BigMem/review inside the harness |
